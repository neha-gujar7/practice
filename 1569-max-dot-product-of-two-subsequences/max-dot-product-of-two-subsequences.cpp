...
public:
    ...
    int solv(vector<int>& nums1, vector<int>& nums2, int i, int j) {
        if(i == m || j == n)
        ...
        int take_i_j = solv(nums1, nums2, i+1, j+1) + val;
        
        int take_i = solv(nums1, nums2, i, j+1);
        int take_j = solv(nums1, nums2, i+1, j);
